...
public class BallWorldStore {
    ...
    public PropertyChangeListener[] updateBallWorld() {
        UpdateStateCmd tempCmd = new UpdateStateCmd(pcs.getPropertyChangeListeners("theClock"));
        pcs.firePropertyChange("theClock", null, tempCmd);
    ...
    public APaintObject loadPaintObj(String type) {
        if(type.equals("ball")){
            Ball tempBall = Ball.makeBall(StraightStrategy.makeStrategy(), dims);
            addListener(tempBall);
            return tempBall;
        }else if(type.equals("wall")){
            Wall tempWall = Wall.makeWall(StraightStrategy.makeStrategy(), dims);
            addListener(tempWall);
            return tempWall;
        }else{
            return null;
        }
